  ...
  var H265Parser = /*#__PURE__*/function () {
    ...
    _createClass(H265Parser, null, [{
      ...
      value: function readSPS(data) {
        ...
        var sps_max_sub_layers_minus1 = decoder.readBits(3); // sps_max_sub_layers_minus1
        ...
        decoder.readUEG();
        decoder.readUEG();
        var ordering_flag = decoder.readBits(1); // sps_sub_layer_ordering_info_present_flag

        for (var _i = ordering_flag ? 0 : sps_max_sub_layers_minus1; _i <= sps_max_sub_layers_minus1; _i++) {
          decoder.readUEG();
          decoder.readUEG();
          decoder.readUEG();
        }
        ...
        var scaling_list_enabled_flag = decoder.readBits(1);
        if (scaling_list_enabled_flag) {
          var sps_scaling_list_data_present_flag = decoder.readBits(1);
          if (sps_scaling_list_data_present_flag) {
            // inline skipScalingListH265
            for (var sizeId = 0; sizeId < 4; sizeId++) {
              for (var matrixId = 0; matrixId < (sizeId === 3 ? 2 : 6); matrixId++) {
                var flag = decoder.readBits(1);
                if (!flag) {
        ...
        decoder.readBits(1);
        decoder.readBits(1);
        var pcm_enabled_flag = decoder.readBits(1);
        if (pcm_enabled_flag) {
        ...
        var num_short_term_ref_pic_sets = decoder.readUEG();
        // inline simplified short-term RPS parsing
        for (var stIdx = 0; stIdx < num_short_term_ref_pic_sets; stIdx++) {
          ...
          if (inter_ref_pic_set_prediction_flag) {
            ...
            var num_negative_pics = decoder.readUEG();
            var num_positive_pics = decoder.readUEG();
            for (var _i2 = 0; _i2 < num_negative_pics; _i2++) {
              decoder.readUEG(); // delta_poc_s0_minus1[i]
              decoder.readBits(1); // used_by_curr_pic_s0_flag[i]
            }
            for (var _i3 = 0; _i3 < num_positive_pics; _i3++) {
              decoder.readUEG(); // delta_poc_s1_minus1[i]
              decoder.readBits(1); // used_by_curr_pic_s1_flag[i]
            }
          }
        }
        var long_term_ref_pics_present_flag = decoder.readBits(1);
        if (long_term_ref_pics_present_flag) {
          var num_long_term_ref_pics_sps = decoder.readUEG();
        ...
        if (vui_parameters_present_flag) {
          ...
          var default_display_window_flag = decoder.readBoolean();
          ...
          var vui_timing_info_present_flag = decoder.readBoolean();
          if (vui_timing_info_present_flag) {
            var vui_num_units_in_tick = decoder.readUInt();
            var vui_time_scale = decoder.readUInt();
            var vui_poc_proportional_to_timing_flag = decoder.readBoolean();
            if (vui_poc_proportional_to_timing_flag) {
              decoder.readUEG(); // vui_num_ticks_poc_diff_one_minus1
            }
            var vui_hrd_parameters_present_flag = decoder.readBoolean();
            if (vui_hrd_parameters_present_flag) {
              // inline simplified HRD parsing
              var commonInfPresentFlag = true;
              ...
              for (var _i5 = 0; _i5 <= sps_max_sub_layers_minus1; _i5++) {
                var fixed_pic_rate_general_flag = decoder.readBoolean();
                ...
                if (nal_hrd_parameters_present_flag) {
                  for (var j = 0; j <= cpb_cnt_minus1; j++) {
                ...
                if (vcl_hrd_parameters_present_flag) {
                  for (var _j = 0; _j <= cpb_cnt_minus1; _j++) {
                    decoder.readUEG(); // bit_rate_value_minus1
                    decoder.readUEG(); // cpb_size_value_minus1
                    if (sub_pic_hrd_params_present_flag) {
                      decoder.readUEG(); // cpb_size_du_value_minus1
          ...
          var bitstream_restriction_flag = decoder.readBoolean();
    ...
    _createClass(RemuxController, [{
      ...
      value: function addTrack(type) {
        ...
        if (type === 'audio' || type === 'both') {
          ...
          this.tracks.audio.on('outOfData', function () {
